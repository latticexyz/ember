...
const NUMBER_OF_TILE_UPGRADES = 7;
const NUMBER_OF_CREATURE_SPECIES = 1;
const NUMBER_OF_CREATURE_TYPES = 5;
const NUMBER_OF_CREATURE_LEVELS = 4;
const NUMBER_OF_ACTION_TYPES = 3;
...
export const Initializers = yup
    .object({
        gameConstants: yup.object({
            ...
            CREATURES_MIN_SECOND_DELAY_BETWEEN_SPAWN: yup.number().min(1).required(),
            ...
            CREATURES_BASE_STAT_PER_SPECIES: yup
                .array(yup.array(yup.number().min(1)).length(2))
                .length(NUMBER_OF_CREATURE_SPECIES)
                .required(),
