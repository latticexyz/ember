...
export interface GameManagerEvents {
  ...
  // claiming tiles
  [GameManagerEvent.ClaimTileScheduled]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileStarted]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileProofDone]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileTXSubmitting]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileTXSubmitted]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileTXConfirmed]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileCancelled]: (coord: WorldCoord) => void;
  [GameManagerEvent.ClaimTileFailed]: (coord: WorldCoord) => void;
...
class GameManager extends createStrictEventEmitterClass<GameManagerEvents>() implements RequiresServices {
  ...
  public setupEventListeners() {
    ...
    this.dungeonEventListeners = [
      ...
      addListener(this.extendedDungeon, DungeonEvent.PlayerUpdated, () =>
        this.actionQueue.process([ActionType.InitiateWallTile, ActionType.UpgradeTile, ActionType.SpawnCreature])
      ),
  ...
  public cancelClaimTile(coords: WorldCoord) {
    const actionId = getActionId(coords, ActionType.ClaimTile);
    this.cancelAction(actionId);
  }
  ...
  public cancelInitiateWallTile(coords: WorldCoord) {
    const actionId = getActionId(coords, ActionType.InitiateWallTile);
    this.cancelAction(actionId);
  }
  ...
  public cancelCompleteWallTile(coord: WorldCoord) {
    const actionId = getActionId(coord, ActionType.CompleteWallTile);
    this.cancelAction(actionId);
  }
  ...
  public async initiateWallTile(coord: WorldCoord) {
    return this.initiateWallTiles([coord]);
  }

  public async initiateWallTiles(coords: WorldCoord[]) {
    const actions: Action<any>[] = coords
      .map((coord) => {
        ...
        return createInitiateWallTileAction({ coord }, this.actionContext);
      })
      .filter(notNull);
  ...
  public async claimTile(coord: WorldCoord) {
    const { pass } = assert([playerIsInitialized(this.actionContext)]);
    if (!pass) return;

    const action = createClaimTileAction({ coord }, this.actionContext);
    action && this.actionQueue.add(action);
  }

  public async claimTiles(coords: WorldCoord[]) {
    if (coords.length === 1) return this.claimTile(coords[0]);
    ...
    const actions: Action<any>[] = [];
    for (const coord of coords) {
      actions.push(createClaimTileAction({ coord }, this.actionContext));
    }

    const action = createMetaAction({ actionGraph: { actions }, name: "Claim tiles" }, this.actionContext);
    action && this.actionQueue.add(action);
  }
  ...
  public processDelayedActions(delayedActions: TileDelayedAction[], initial?: boolean) {
    for (const delayedAction of delayedActions) {
      ...
      if (initiator === this.address) {
        const createAction = {
          [TileDelayedActionType.WALL]: createCompleteWallTileAction,
