...
export const createTileAction: ActionCreator<CreateTileActionData, Action<WorldCoord[] | null>> = (
  ...
  const events = actionEvents[type] as
    | typeof actionEvents[ActionType.MineTile]
    | typeof actionEvents[ActionType.ClaimTile];
  ...
  const action = new Action({
    ...
    name,
    costByResource: {
      [ResourceType.Mana]: constants.gameConstants.MANA_PER_ACTION_TYPE[ActionTypeToContractActionType[type]],
    },
    onStateChange: (state) => {
      if (!emit) return;
      ...
      const { submitted, confirmed } = txExecutor.makeRequest({
        ...
        genTransaction: async (nonce, _) => {
          const ethMethod = {
            ...
            [ActionType.ClaimTile]: net.contracts.dungeonFacet.claimTile,
          };
