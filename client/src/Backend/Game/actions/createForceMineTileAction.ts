...
export const createForceMineTileAction: ActionCreator<CreateForceMineTileActionData, Action<WorldCoord[] | null>> = (
  ...
  const manaCost = type === ActionType.CompleteForceMineTile ? constants.gameConstants.MANA_PER_ACTION_TYPE[ContractActionType.MINE] : 0;
  ...
  const action = new Action({
    id: actionId,
    type,
    name: `Force mine (${type === ActionType.InitiateForceMineTile ? "Initiate" : "Complete"}) ${coord.x}/${coord.y}`,
    costByResource: {
