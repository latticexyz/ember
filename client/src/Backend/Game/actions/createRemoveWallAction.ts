...
import { tileIsMined, playerControlsRegion, tileIsOwnedByPlayer, tileIsWalled } from "./assertions";
...
interface RemoveWallData {
  coord: WorldCoord;
}

export const createRemoveWallAction: ActionCreator<RemoveWallData, Action<boolean | null>> = ({ coord }, context) => {
  const { extendedDungeon, emit, txExecutor, net, movingAverages, player } = context;
  const actionId = getActionId(coord, ActionType.RemoveWall);
  const events = actionEvents[ActionType.RemoveWall];
  const action = new Action({
    id: actionId,
    type: ActionType.RemoveWall,
    name: `Remove wall ${coord.x}/${coord.y}`,
    onStateChange: (state) => {
      if (!emit) return;
    ...
    process: async () => {
      ...
      const { submitted, confirmed } = txExecutor.makeRequest({
        ...
        genTransaction: async (nonce, _) => {
          ...
          return net.contracts.dungeonFacet.removeWall(tileId, {
            nonce,
            ...net.ethersOverrides,
