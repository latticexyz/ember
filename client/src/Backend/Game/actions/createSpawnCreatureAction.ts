...
export const createSpawnCreatureAction: ActionCreator<SpawnCreatureData, Action<boolean | null>> = (
  ...
  const goldCost = constants.gameConstants.CREATURES_PRICE[creatureSpecies][creatureType][Resource.GOLD];
  const soulCost = constants.gameConstants.CREATURES_PRICE[creatureSpecies][creatureType][Resource.SOULS];
  ...
  const action = new Action({
    ...
    costByResource: {
      // those semaphore just don't work :/
    ...
    requirement: () => {
      ...
      if (goldCost > playerData.gold || soulCost > playerData.souls) {
        return false;
      }
