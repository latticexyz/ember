...
const PRIORITY_BY_TYPE: PriorityByActionType = {
  ...
  [ActionType.UpgradeSettlement]: 4,
  [ActionType.HarvestTiles]: 2,
  [ActionType.WallTile]: 2,
  ...
  [ActionType.CreateSettlement]: 7,
  [ActionType.DestroySettlement]: 7,
  [ActionType.RemoveUpgrade]: 7,
  [ActionType.RemoveWall]: 7,
};
...
export class ActionQueue extends createStrictEventEmitterClass<ActionEvents>() {
  ...
  resourceSemaphores: { [type in ResourceType | string]?: ResourceSemaphore };
  ...
  private async acquireSemaphore(action: Action<any>): Promise<boolean> {
    ...
    hadToWait = await this.semaphore.acquire(1);
    hadToWait = (await this.actionSemaphores[action.type]?.acquire(1)) || hadToWait;
  ...
  private async releaseSemaphore(action: Action<any>): Promise<void> {
    ...
    await this.actionSemaphores[action.type]?.release(1);
    await this.semaphore.release(1);
  }
