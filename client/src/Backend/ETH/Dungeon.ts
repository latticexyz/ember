...
class Dungeon extends createStrictEventEmitterClass<DungeonEvents>() {
  ...
  public settlements: CoordMap<Settlement>;
  ...
  public constructor(net: Network<GameContracts, TxType>, player: EthAddress, constants: Constants) {
    ...
    this.settlements = new CoordMap<Settlement>();
    ...
    makeObservable(this, {
      ...
      updateCurrentManaFromChainTime: action,
      chargeManaForAction: action,
  ...
  public setSettlements(settlements: CoordMap<Settlement>) {
    this.settlements = settlements;
  }

  public setSettlement(coord: WorldCoord, settlement: Settlement) {
    this.settlements.set(coord, settlement);
  }
  ...
  private optimisticallyIncreaseMana(address: EthAddress, amount: number) {
    const player = this.players.get(address);
    if (!player) return;
    ...
    if (amount > 0) {
      this.players.set(address, {
        ...player,
        mana: newMana > maxMana ? maxMana : newMana,
        lastManaUpdateTimestamp: this.net.predictedChainTime,
      });
    }
    this.emit(DungeonEvent.PlayerUpdated, player);
  }

  private optimisticallyDecreaseMana(address: EthAddress, amount: number) {
    const player = this.players.get(address);
    if (!player) return;
    const newMana = player.mana - amount;
    this.players.set(address, {
  ...
  public chargeManaForAction(address: EthAddress, cost: number) {
    const player = this.players.get(address);
    if (!player) return false;

    this.optimisticallyDecreaseMana(address, cost);
  }

  public updateCurrentManaFromChainTime(address: EthAddress) {
    const player = this.players.get(address);
    if (!player) return;
    ...
    this.optimisticallyIncreaseMana(address, manaIncrease);
  }
  ...
  public async load(): Promise<void> {
    ...
    this.logToEventAndConsole("Getting settlements...");
    this.emit(DungeonEvent.LoadingStage, DungeonLoadingStage.REGIONS);
  ...
  private processSettlementUpdatedEvent({ region: _regionId, data: _data }: SettlementUpdatedParams) {
    const regionCoord = idToCoord(_regionId);
    const settlement: Settlement = SettlementFromContractData(_data);
    this.setSettlement(regionCoord, settlement);
    this.emit(DungeonEvent.SettlementUpdated, settlement, regionCoord);
  }
  ...
  private processCreatureUpdatedEvent({
    ...
    console.log("received a creature updated event");
    const regionCoord = idToCoord(_regionId);
    const creatureId = _creatureId.toString();
    const creature = CreatureFromContractData(_data);
    this.creatures.set(creatureId, creature);
  ...
  private getEventHandlers(): {
    ...
    const dungeonFacetEventHandlers: DungeonFacetEventHandlers = {
      ...
      [DungeonFacetEvent.SettlementUpdated]: this.processSettlementUpdatedEvent.bind(this),
  ...
  protected setupEventListeners(): void {
    ...
    const dungeonFacetTopics = [
      [
        ...
        dungeonFacet.filters.SettlementUpdated(null, null).topics,
  ...
  private async getSettlements(): Promise<CoordMap<Settlement>> {
    const nSettlements: number = (
      await this.makeCall<EthersBN>(this.net.bulkContracts.getterFacet.getNSettlements)
    ).toNumber();

    const settlementCoord: WorldCoord[] = await aggregateBulkGetter<WorldCoord>(
      nSettlements,
      OBJECT_PER_QUERY_BULK_LOADING,
      async (start, end) =>
        (
          await this.makeCall<BigNumberish[]>(this.net.bulkContracts.getterFacet.bulkGetSettlementIds, [start, end])
        ).map((b: BigNumberish) => idToCoord(b)),
      this.createLogPercentageFunction("getting settlement coordinates")
    );

    const settlements = await aggregateBulkGetter<Settlement>(
      nSettlements,
      OBJECT_PER_QUERY_BULK_LOADING,
      async (start, end) =>
        (
          await this.makeCall<
            {
              ...
              level: BigNumber;
              energy: BigNumber;
              lastEnergyUpdateTimestamp: BigNumber;
            }[]
          >(this.net.bulkContracts.getterFacet.bulkGetSettlementByIds, [
            settlementCoord.slice(start, end).map((t) => coordToId(t)),
          ])
        ).map(SettlementFromContractData),
      this.createLogPercentageFunction("getting settlements")
    );

    const settlementMap = new CoordMap<Settlement>();

    for (const [index, regionCoord] of settlementCoord.entries()) {
      settlementMap.set(regionCoord, settlements[index]);
    }
