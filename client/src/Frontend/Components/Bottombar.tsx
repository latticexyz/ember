...
const UpgradeBtn: React.FC<UpgradeBtnProps> = ({ upgradeItem, upgradeKey, spritesheetParams }) => {
  ...
  return (
    <Tooltip
      ...
      stats={[
        ["Gold", toolInfo.GOLD_PRICES[upgradeItem]],
        ["Mana", toolInfo.MANA_PRICE],
      ]}
      ...
      {upgradeItem === UpgradeItem.SoulGenerator && !doesHaveHarvestableResource
        ...
        <UpgradeContainer
          onClick={() => {
            ...
            UIManager.getInstance().state.setShowHotbar(false);
          }}
        >
          <UpgradeContent className="upgrade-content">
            <Spritesheet spriteWidth={24} {...spritesheetParams} timePerFrame={"0.8s"} />
      ...
      <Container>
        {[...UpgradesToSpriteSheetsParam.entries()].map(([upgrade, spritesheetParams], i) => (
          <MetaContainer key={"upgrade-" + i}>
            <UpgradeBtn upgradeItem={upgrade} upgradeKey={i} spritesheetParams={spritesheetParams} />
          </MetaContainer>
        ))}
      </Container>
    </GameUI>
