...
import { claim, creature, mine, upgrade } from "../../Assets/tools";
import { ActionTypeToContractActionType } from "../../Backend/Utils/Utils";
import { colors, fonts } from "../../theme";
import { ActionType } from "../../_types/GlobalTypes";
...
export const InputStatusToast = observer(() => {
  ...
  const { selectedCoords, mineableCoords, claimableCoords, upgradeableCoords, forceMineCoords, selectedCreatureIds } =
    selectionManager;

  const upgradeManaCost = gm?.constants.gameConstants.MANA_PER_ACTION_TYPE[ActionTypeToContractActionType[ActionType.UpgradeTile]] || 0;
  const claimManaCost = gm?.constants.gameConstants.MANA_PER_ACTION_TYPE[ActionTypeToContractActionType[ActionType.ClaimTile]] || 0;
  const mineManaCost = gm?.constants.gameConstants.MANA_PER_ACTION_TYPE[ActionTypeToContractActionType[ActionType.MineTile]] || 0;
        ...
        (mineableCoords.length > 0 ||
          ...
          <StatusContainer>
            ...
            <InfoContainer>
              ...
              {claimableCoords.length > 0 && (
                <ToastItem text={`Claim ${claimableCoords.length} tiles
                (${claimableCoords.length}x${claimManaCost}=${claimableCoords.length * claimManaCost} Mana)`} action="claim" icon={claim} />
              )}
