...
import { creatureToSpritesheet } from "../../SummonCreatures/constants";
...
export const CreatureListItem: React.FC<{ creature: Creature; boost: number; details?: boolean; onClick: () => void }> =
  ({ creature, details, boost, onClick }) => {
    ...
    const maxLife =
      constants.CREATURES_BASE_STAT_PER_SPECIES[creature.species][CreatureStat.LIFE] *
      (isUnique ? constants.CREATURES_UNIQUE_STAT_MULTIPLIER : 1);
    const spritesheet = creatureToSpritesheet[creature.species][creature.creatureType];
    const attack =
      constants.CREATURES_BASE_STAT_PER_SPECIES[creature.species][CreatureStat.ATK] *
      (isUnique ? constants.CREATURES_UNIQUE_STAT_MULTIPLIER : 1);
