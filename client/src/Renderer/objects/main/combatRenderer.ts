...
export default class CombatRenderer extends Phaser.GameObjects.Container {
  ...
  squad1Stars: Phaser.GameObjects.Sprite[];
  squad2: Creature[];
  squad2Sprites: Phaser.GameObjects.Sprite[];
  squad2Stars: Phaser.GameObjects.Sprite[];
  ...
  constructor(
    ...
    // init the combat
    const [squad1Sprites, squad1LifeBars, squad1Stars] = this.addSquadSprites(this.squad1, true);
    const [squad2Sprites, squad2LifeBars, squad2Stars] = this.addSquadSprites(this.squad2, false);
    ...
    this.squad1Stars = squad1Stars;
    this.squad2Stars = squad2Stars;
    this.crosses = [];
  ...
  async playHalfRound(halfRoundTrace: HalfRoundTrace, squad: number) {
    ...
    for (const [i, a] of halfRoundTrace.entries()) {
      ...
      const currentSquadSprites = squad === 0 ? this.squad1Sprites : this.squad2Sprites;
      const currentSquad = squad === 0 ? this.squad1 : this.squad2;
      const currentCreatureSprite = currentSquadSprites[a.initiator];
      const currentCreature = currentSquad[a.initiator];
      const sprite = creatureToSprite[currentCreature.species][currentCreature.creatureType];
      currentCreatureSprite.anims.play(sprite + "-" + SpriteModifier.Action);
      currentCreatureSprite.on("animationcomplete", () => {
        currentCreatureSprite.play(sprite + "-" + SpriteModifier.Idle);
      });
      await sleep(300 / this.accelerationFactor);
  ...
  async setCreatureAsDead(squad: number, squadIndex: number, skipTweening = false) {
    ...
    const stars = squad === 0 ? this.squad1Stars : this.squad2Stars;
    this.scene.tweens.add({
      targets: stars[squadIndex],
      alpha: 0,
      duration,
      ease: Phaser.Math.Easing.Quadratic.InOut,
    });
  ...
  addSquadSprites(
    squad: Creature[],
    isSquad1: boolean
  ): [Phaser.GameObjects.Sprite[], LifeBar[], Phaser.GameObjects.Sprite[]] {
    ...
    for (const [i, c] of squad.entries()) {
      ...
      this.add(lb);
      lbOut.push(lb);
      const star = new Phaser.GameObjects.Sprite(this.scene, pos.x + TILE_WIDTH / 2, pos.y - 8, "");
      this.add(star);
      stars.push(star);
      const { level } = c;
      if (level === 3) {
        star.setVisible(true);
        star.anims.play("stars-gold", true);
      } else if (level === 2) {
        star.setVisible(true);
        star.anims.play("stars-silver", true);
      } else if (level === 1) {
        star.setVisible(true);
        star.anims.play("stars-bronze", true);
      } else {
        star.anims.stop();
  ...
  createCreatureSprite(c: Creature, x: number, y: number): Phaser.GameObjects.Sprite {
    ...
    s.anims.play(sprite + "-" + SpriteModifier.Idle, true);
    s.addToUpdateList();
    return s;
  }
  buildCombatFrame() {
