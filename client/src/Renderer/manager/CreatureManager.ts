...
import { Creature as CreatureData, EthAddress, WorldCoord } from "../../_types/GlobalTypes";
...
export class CreatureManager implements Service {
  ...
  private registerCreatures(creatureIds: string[]) {
    ...
    for (let i = 0; i < creatureIds.length; i++) {
      ...
      this.registerCreature(creatureIds[i], location);
    }
  }
   ...
   */
  public registerCreature(id: string, initialTileCoord: WorldCoord) {
      ...
      new ViewportGameObject(id, {
        spawn: (currentCoord) => {
          ...
          if (!currentCoord || this.gm.extendedDungeon.getTileAt(currentCoord).upgrade !== TileUpgrade.NONE)
            this.setDestination(id, initialTileCoord, creatureOwner, true);
    ...
    this.unitMoveManager.registerUnit(id, UnitType.Creature, creature.owner);
  }
  ...
  public updateCreatureLife(id: string, life: number, maxLife: number) {
    const creature = this.getCreatureById(id);
    creature?.updateLife(life, maxLife);
  }

  public updateCreatureLevel(id: string, level: number) {
    const creature = this.getCreatureById(id);
    creature?.updateLevel(level);
  }
  ...
  private getReachableTilesInRegion(destinationTile: WorldCoord, creatureOwner: EthAddress) {
    ...
    const reachableCoordsInDestinationRegion = getSurroundingTilesOfSameType(
      ...
      (tile, coord) => {
        ...
        return (
          tile.isMined &&
          (!tile.isWalled || tile.owner === creatureOwner) &&
  ...
  public setDestination(id: string, destinationTile: WorldCoord, creatureOwner: EthAddress, randomEqualTile?: boolean) {
    let toOptions = randomEqualTile
      ? this.getReachableTilesInRegion(destinationTile, creatureOwner)
      : [destinationTile];
    ...
    this.setPlannedDestination(id, undefined, creatureOwner);
    this.unitMoveManager.setPath(id, path);
  }

  public setPlannedDestination(creatureId: string, destinationTile: WorldCoord | undefined, creatureOwner: EthAddress) {
    ...
    if (!destinationTile) {
      ...
      const isTraversable = (coord: WorldCoord): boolean => {
        const tile = this.gm.extendedDungeon.getTileAt(coord);
        return !!tile && tile.isMined && (!tile.isWalled || tile.owner === creatureOwner);
      };
