...
export class GameSelection extends BaseSelection {
  ...
  setSelectedCoords(coords: WorldCoord[]) {
    ...
    if (coords.length === 0) {
      ...
      this.setTerrainCoords([], TerrainType.Claim);
  ...
  setTerrainCoords(coords: WorldCoord[], terrainType: TerrainType) {
    ...
    if (terrainType === TerrainType.Claim) {
      this.claimableCoords = coords;
    }
  ...
  hasClaimableCoords() {
    return this.claimableCoords.length > 0;
  }
