...
export class ToolManager implements Service {
  ...
  bootService(scene: MainScene) {
    this.tools = {
      ...
      mine: new MineTool(this.gm, this.ui.state),
    ...
    this.tools.mine.bootService(scene);
  ...
  destroyService() {
    ...
    this.tools.mine.destroyService();
  ...
  public getGameObjects(): Phaser.GameObjects.GameObject[] {
    return [this.tools.interact.selection, /** this.tools.mine.mineableArea, **/ this.tools.creature.mask.background];
  }
  ...
  public handleAction() {
    if (this.selection.hasMineableCoords() || this.selection.hasForceMineCoords()) {
      this.tools.mine.mineTiles(this.selection.mineableCoords, this.selection.forceMineCoords);
    }

    if (this.selection.hasUpgradeableCoords()) {
      this.ui.state.setShowHotbar(true, tileCoordToRegionCoord(this.selection.upgradeableCoords[0]));
    }
  }

  public handleCancel() {
    if (this.selection.hasMineableCoords() || this.selection.hasForceMineCoords()) {
      this.tools.mine.cancelMineTiles(this.selection.mineableCoords, this.selection.forceMineCoords);
    }
    if (this.selection.hasUpgradeableCoords()) {
  ...
  public handleUpgrade(upgrade: UpgradeItem) {
    ...
    if (this.ui.state.showHotbar) {
      this.ui.state.setShowHotbar(false, { x: 0, y: 0 });
    }
  ...
  public updateToolGraphics() {
    if (this.selection.hasMineableCoords()) {
      this.tools.mine.setAvailableArea();
    }
  }
