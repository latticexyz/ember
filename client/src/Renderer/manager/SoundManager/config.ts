...
export function SoundConfig(type: SoundType, scene: Phaser.Scene): Phaser.Sound.BaseSound | undefined {
  ...
  if (type === SoundType.CLAIM) {
    const rate = getRandomNumberBetween(0.75, 2);
    sound = scene.sound.add(SoundType.CLAIM, { loop: true, rate });
    // const createSound = getRandomElement([
    //   () => scene.sound.add(SoundType.CLAIM, { loop: true, rate: 1, volume: 0.5 }),
    //   () => scene.sound.add(SoundType.CLAIM, { loop: true, rate: 1.5, volume: 0.5 }),
    //   () => scene.sound.add(SoundType.CLAIM, { loop: true, rate: 2, volume: 0.5 }),
    //   () => scene.sound.add(SoundType.CLAIM, { loop: true, rate: 0.75, volume: 0.5 }),
    // ]);
    // sound = createSound();
  }
  ...
  if (type === SoundType.CLAIM_END) {
    const detune = getRandomNumberBetween(-100, 100);
    sound = scene.sound.add(SoundType.CLAIM_END, { loop: false, detune });
  }
