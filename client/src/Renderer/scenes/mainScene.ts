...
import { isRockWall, isTileWall } from "../utils/tiles";
...
export default class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.tilemapAnimator = new TilemapAnimator(100, [
      // Soul generator
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.SoulGenerator]!,
      },
      // Soul storage
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.SoulStorage]!,
      },
      // Gold generator
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.GoldGenerator]!,
      },
      // Lair
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.Lair]!,
      },
      // Dungeon heart A
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.DHa]!,
      },
      // Dungeon heart B
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.DHb]!,
      },
      // Dungeon heart C
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.DHc]!,
      },
      // Dungeon heart D
      {
        frames: ObjectTilesetAnimations[ObjectTilesetId.DHd]!,
      },
    ]);
  ...
  setupAnimations() {
    const animationNames = ["imp", "skeleton", "fire-skeleton", "ice-skeleton", "demon-skeleton", "beholder"];
    for (const animName of animationNames) {
      this.anims.create({
        key: animName + "-idle",
        frames: this.anims.generateFrameNumbers(animName, { start: 0, end: 3 }),
        frameRate: 10,
        repeat: -1,
      });
    }
  ...
  setupEventListeners() {
    ...
    this.dungeonEventListeners = [
      ...
      addListener(this.gm.extendedDungeon, DungeonEvent.TileClaimed, async (tile, coord) => {
        ...
        this.phaserManager.services.strategicMapChunkManager.setTileStale(coord);
        this.phaserManager.services.toolManager.updateToolGraphics();
        SoundManager.register(SoundType.CLAIM_END, coord);
      }),
      ...
      addListener(this.gm.extendedDungeon, DungeonEvent.CreatureMovedToRegion, (creatureId, creature, tileCoord) => {
        ...
        if (existingCreature) {
          this.phaserManager.services.creatureManager.setDestination(creatureId, tileCoord);
          this.phaserManager.services.creatureManager.updateCreatureLife(creatureId, creature.life);
        } else {
          this.phaserManager.services.creatureManager.registerCreature(creatureId, creature, tileCoord);
      ...
      addListener(
        ...
        (regionCoord, oldController, newController) => {
          ...
          for (const t of tilesInRegion(regionCoord)) {
            ...
            if (isRockWall(tileIndex) || isTileWall(tileIndex)) {
    ...
    const tileScheduledEvents: GameManagerEvent[] = [
      ...
      GameManagerEvent.ClaimTileScheduled,
    ...
    const tileTXSubmittingEvents: GameManagerEvent[] = [
      ...
      GameManagerEvent.ClaimTileTXSubmitting,
    ...
    const tileTXSubmittedEvents: GameManagerEvent[] = [
      ...
      GameManagerEvent.ClaimTileTXSubmitted,
    ...
    const tileTXCancelledEvents: GameManagerEvent[] = [
      ...
      GameManagerEvent.ClaimTileCancelled,
    ...
    const tileTXConfirmedEvents: GameManagerEvent[] = [
      ...
      GameManagerEvent.ClaimTileTXConfirmed,
    ...
    const tileFailedEvents: GameManagerEvent[] = [
      ...
      GameManagerEvent.ClaimTileFailed,
    ...
    this.gameManagerEventListeners = [
      addListener(this.gm, GameManagerEvent.MoveCreaturesScheduled, (creatureIds, destinationTile) => {
        for (const creatureId of creatureIds) {
          this.phaserManager.services.creatureManager.setPlannedDestination(creatureId, destinationTile);
        }
      }),

      addListener(this.gm, GameManagerEvent.MoveCreaturesCancelled, (creatureIds) => {
        for (const creatureId of creatureIds) {
          this.phaserManager.services.creatureManager.setPlannedDestination(creatureId, undefined);
        }
      }),
      ...
      addListener(this.gm, GameManagerEvent.ClaimTileStarted, (coord) => {
        tileEventHandlers.started(coord, ImpWorkType.CLAIM);
      }),
    ...
    this.uiEventListeners = [
      ...
      addListener(this.uiManager, UIEvent.SendSingleActionMessage, (action) => {
        if (action === "mine" && this.phaserManager.services.selectionManager.state.hasMineableCoords()) {
          this.phaserManager.services.toolManager.tools.mineOrClaim.mineOrClaimTiles(this.phaserManager.services.selectionManager.state.mineableCoords, [], []);
          clearSelection(this.phaserManager.services.selectionManager.state, this.phaserManager.services.toolManager);
        }
        if (action === "claim" && this.phaserManager.services.selectionManager.state.hasClaimableCoords()) {
          this.phaserManager.services.toolManager.tools.mineOrClaim.mineOrClaimTiles([], this.phaserManager.services.selectionManager.state.claimableCoords, []);
          clearSelection(this.phaserManager.services.selectionManager.state, this.phaserManager.services.toolManager);
        }
        if (action === "forceMine" && this.phaserManager.services.selectionManager.state.hasForceMineCoords()) {
          this.phaserManager.services.toolManager.tools.mineOrClaim.mineOrClaimTiles([], [], this.phaserManager.services.selectionManager.state.forceMineCoords);
          clearSelection(this.phaserManager.services.selectionManager.state, this.phaserManager.services.toolManager);
        }
        if (action === "upgrade" && this.phaserManager.services.selectionManager.state.hasUpgradeableCoords()) {
          this.uiManager.state.setShowHotbar(true);
        }
      }),
